...
emit_client_code(void *drcontext, client_info_t *client, byte *pc)
{
    ...
    dr_insert_clean_call(drcontext, ilist, instrlist_first(ilist), clean_call, false, 0);
    instrlist_meta_append(
        ilist,
        INSTR_CREATE_jmp_ind(drcontext, OPND_CREATE_ABSMEM(&client->retaddr, OPSZ_PTR)));
    ...
    for (; instr != NULL; instr = instr_get_next(instr)) {
        ...
        case OP_cli: {
            ...
            instrlist_meta_preinsert(
                bb, instr,
                INSTR_CREATE_mov_st(
                    drcontext, OPND_CREATE_ABSMEM((byte *)&client->retaddr, OPSZ_PTR),
                    opnd_create_instr(label)));
            instrlist_meta_preinsert(
                bb, instr,
                INSTR_CREATE_jmp(drcontext, opnd_create_pc(client->cache_start)));
...
{
    cpu_client_info = kzalloc(dr_cpu_count() * sizeof(client_info_t *), GFP_KERNEL);
