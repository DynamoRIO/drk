...
generic_page_table_entry_t *
follow_page_table_entry(generic_page_table_entry_t *entry)
{
    return (generic_page_table_entry_t *)phys_to_virt(PFN_TO_PA((uint64)entry->next_pfn));
}

generic_page_table_entry_t *
get_l4_page_table(void)
{
    cr3_t cr3;
    get_cr3(&cr3);
    return (generic_page_table_entry_t *)(uint64)phys_to_virt(PFN_TO_PA(cr3.l4_pfn));
}

bool
page_table_get_page(generic_page_table_entry_t *l4, void *virtual_address,
                    vm_region_t *region, unsigned long *pfn,
                    generic_page_table_entry_t **parent, int *parent_level)
{
...
bool
page_table_get_physical_address(generic_page_table_entry_t *l4, void *virtual_address,
...
static void
get_region_callback(const vm_region_t *region, void *void_arg)
{
    get_region_arg_t *arg = (get_region_arg_t *)void_arg;
    if (arg->address_to_find >= region->start && arg->address_to_find <= region->end) {
        *arg->output = *region;
        arg->found = true;
...
void
page_table_get_region(generic_page_table_entry_t *l4, void *virtual_address,
                      vm_region_t *region)
{
...
static void
depth_first_traverse_page_table_recursive(generic_page_table_entry_t *table, int level,
    ...
    for (i = 0; i < get_page_table_size(table); i++) {
        ...
        if (entry->present) {
            ...
            if (level == 1) {
                ...
                pfns = 0;
                depth_first_traverse_page_table_recursive(follow_page_table_entry(entry),
                                                          level - 1, callback, arg);
            }
            for (j = 0; j < pfns; j++) {
                /* Leaves. */
...
static bool
address_is_canonical(void *address)
{
    return address < VM_HOLE_START || address > VM_HOLE_END;
}

static void
non_canonical_address(void)
{
}

static void
traverse_page_table_recursive(generic_page_table_entry_t *table, int level,
                              bool truncate_nx, vm_region_t *parent,
                              page_table_traversal_callback_t callback, void *arg)
{
    ...
    for (i = 0; i < get_page_table_size(table); i++) {
        ...
        child.access.executable = parent->access.executable & !entry->not_executable;
        ...
        if ((truncate_nx && entry->not_executable) || (!entry->present) || (level == 1) ||
            (level == 2 && entry->size) || (level == 3 && entry->size)) {
            callback(&child, arg);
        } else {
            traverse_page_table_recursive(follow_page_table_entry(entry), level - 1,
                                          truncate_nx, &child, callback, arg);
        }
    }
}

void
traverse_page_table(generic_page_table_entry_t *l4, bool truncate_nx,
                    page_table_traversal_callback_t callback, void *arg)
{
    ...
    traverse_page_table_recursive(l4, 4, truncate_nx, &start_region, callback, arg);
}
...
static void
contiguous_callback(const vm_region_t *region, void *arg)
{
...
void
traverse_page_table_contiguous(generic_page_table_entry_t *l4,
                               page_table_traversal_callback_t callback, void *arg)
{
...
static bool
can_access_without_exception(generic_page_table_entry_t *l4, void *start, size_t size,
                             bool write)
{
    ...
    for (;;) {
         ...
         * and stops early. */
        if (!page_table_get_page(l4, start, &region, &pfn, &parent, &parent_level)) {
            return false;
        }
...
bool
page_table_readable_without_exception(generic_page_table_entry_t *l4, void *start,
                                      size_t size)
{
...
bool
page_table_writable_without_exception(generic_page_table_entry_t *l4, void *start,
                                      size_t size)
{
