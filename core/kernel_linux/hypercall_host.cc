...
public:
    virtual void
    Open(int fd, const string &path) = 0;
    virtual void
    Close(int fd) = 0;
    virtual void
    Write(int fd, const char *data, int count) = 0;
    virtual void
    Flush(int fd) = 0;
    virtual ~FileSystem()
    {
...
public:
    ...
    void
    Write(int fd, const char *data, int count)
    {
...
private:
    void
    CreateFile(int fd, const string &path)
    {
    ...
    void
    InitParentDirectory(const string &root)
    {
        string time = GetTime();
        ...
        if (mkdir(parent_.c_str(), 0750) != 0) {
            stringstream error;
            error << "Could not create parent directory " << parent_ << ": "
                  << strerror(errno);
...
public:
    DualFileSystem(FileSystem *a, FileSystem *b)
        : a_(a)
        , b_(b)
    {
    }
...
public:
    ...
    void
    HandleHypercall(const hypercall_t &hypercall)
    {
        switch (hypercall.type) {
        case HYPERCALL_NOP: HandleNOP((const hypercall_nop_t &)hypercall); break;
        case HYPERCALL_INIT: HandleInit((const hypercall_init_t &)hypercall); break;
        case HYPERCALL_OPEN: HandleOpen((const hypercall_open_t &)hypercall); break;
        case HYPERCALL_CLOSE: HandleClose((const hypercall_close_t &)hypercall); break;
        case HYPERCALL_WRITE: HandleWrite((const hypercall_write_t &)hypercall); break;
        case HYPERCALL_FLUSH: HandleFlush((const hypercall_flush_t &)hypercall); break;
        default: throw runtime_error("Unknown hypercall type.");
        }
    }

private:
    void
    HandleNOP(const hypercall_nop_t &hc)
    {
    ...
    void
    HandleInit(const hypercall_init_t &hc)
    {
        delete file_system_;
    ...
    void
    HandleOpen(const hypercall_open_t &hc)
    {
        file_system_->Open(hc.fd, &hc.fname);
    }

    void
    HandleClose(const hypercall_close_t &hc)
    {
        file_system_->Close(hc.fd);
    }

    void
    HandleWrite(const hypercall_write_t &hc)
    {
        file_system_->Write(hc.fd, &hc.buffer, hc.count);
    }

    void
    HandleFlush(const hypercall_flush_t &hc)
    {
