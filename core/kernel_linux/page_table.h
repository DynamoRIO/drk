...
static inline int
get_page_table_size(generic_page_table_entry_t *table)
{
    return PAGE_TABLE_ENTIRES_PER_LEVEL;
}

extern generic_page_table_entry_t *
...
static inline bool
vm_region_has_same_permissions(const vm_region_t *a, const vm_region_t *b)
{
    return (!a->present && !b->present) ||
        (a->present && b->present && a->access.writable == b->access.writable &&
         a->access.executable == b->access.executable &&
         a->access.user == b->access.user);
}

static inline bool
vm_region_is_executable(const vm_region_t *region)
{
    return region->present && region->access.executable;
}

static inline bool
vm_region_are_adjacent(const vm_region_t *a, const vm_region_t *b)
{
