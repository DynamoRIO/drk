...
alloc_page_table(void)
{
    generic_page_table_entry_t *table = (generic_page_table_entry_t *)mmap(
        NULL, PAGE_TABLE_SIZE, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_SHARED, -1, 0);
...
void
free_page_table(generic_page_table_entry_t *table)
{
...
void
vector_init(vector_t *vector, unsigned long elem_size)
{
    vector->size = 0;
    vector->capacity = 0;
    vector->data = NULL;
...
void
vector_append(vector_t *vector, void *elem)
{
...
void *
vector_get(vector_t *vector, size_t i)
{
...
void
region_recorder_callback(const vm_region_t *region, void *arg)
{
...
void
pfn_recorder_callback(unsigned long pfn, void *arg)
{
    vector_t *vector = (vector_t *)arg;
    vector_append(vector, (void *)&pfn);
}

#define assert_sequential(first, seq_start, n)   \
    do {                                         \
        int i;                                   \
        for (i = 0; i < n; i++) {                \
...
main()
{
...

    {
        ...
        assert(
            page_table_get_page(l4, (void *)(2), &region, &pfn, &parent, &parent_level));
        assert(region.present);
        ...
        assert(page_table_get_page(l4, (void *)(_1GB + 2), &region, &pfn, &parent,
                                   &parent_level));
        assert(region.present);
        ...
        assert(page_table_get_page(l4, (void *)(_1GB + _4KB * 2), &region, &pfn, &parent,
                                   &parent_level));
        assert(region.present);
        assert(region.start == (void *)_1GB);
        assert(region.end - region.start + 1 == _2MB);
        ...
        assert(page_table_get_page(l4, (void *)(_1GB + _2MB + 2), &region, &pfn, &parent,
                                   &parent_level));
        assert(region.present);
        ...
        assert(!page_table_get_page(l4, (void *)(_1GB + _2MB + _4KB), &region, &pfn,
                                    &parent, &parent_level));
        assert(!region.present);
        assert(parent_level == 1);
        assert(parent == &l1[1]);
        ...
        va.l3_index = 2;
        assert(!page_table_get_page(l4, va.virtual_address, &region, &pfn, &parent,
                                    &parent_level));
        assert(!region.present);
        assert(parent_level == 3);
        assert(parent == &l3[2]);
        ...
        assert(
            page_table_readable_without_exception(l4, (void *)(0), _1GB + _2MB + _4KB));
        assert(
            page_table_writable_without_exception(l4, (void *)(0), _1GB + _2MB + _4KB));
        assert(!page_table_readable_without_exception(l4, (void *)(0),
                                                      _1GB + _2MB + _4KB + 1));
        assert(!page_table_readable_without_exception(l4, (void *)(0), _1GB * 3));
        assert(!page_table_writable_without_exception(l4, (void *)(0),
                                                      _1GB + _2MB + _4KB + 1));
    ...
    /* Test some tricky bits with the hole in the VM. */
    {
        ...
        assert(!page_table_get_page(l4, VM_HOLE_START, &region, &pfn, &parent,
                                    &parent_level));
        assert(parent == NULL);
        assert(!page_table_get_page(l4, VM_HOLE_START + 10, &region, &pfn, &parent,
                                    &parent_level));
        assert(parent == NULL);
        assert(
            !page_table_get_page(l4, VM_HOLE_END, &region, &pfn, &parent, &parent_level));
        assert(parent == NULL);
